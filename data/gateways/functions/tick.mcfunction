...
execute as @e[type=item,limit=1,nbt={OnGround: 1b},nbt={Item: {id: "minecraft:dragon_breath", Count: 1b, tag: {display: {Name: '{"text":"Exact Gateway Essence"}'}}}}] at unless data entity @s Item.tag.Placement @s if block ~ ~ ~ air run function gateways:bottle_gateway
